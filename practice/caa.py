...
def init_model(shape):
    ...
    if shape == "ridges":
        for i in range(size):
            for j in range(size):
                if i - int(size*0.4) <= j <= int(size*1.4) - i and int(size*0.6)-i<=j<int(size*0.4)+i:
                    model[i][j]=1
                    area=area+1
    if shape == "streamline":
        for i in range(size):
            for j in range(size):
                if (i-size/2)**2/((size*0.3)**2)+(j-size/2)**2/((size*0.4)**2)<1.1 and i<int(size*0.4):
                    model[i][j]=1
                    area=area+1
                if i>=int(size*0.4) and 2*i-size*0.9<j<-2*i+size*1.1:
                    model[i][j]=1
                    area=area+1
    ...
    if shape == "oval":
        for i in range(size):
            for j in range(size):
                if (i-size/2)**2/((size*0.3)**2)+(j-size/2)**2/((size*0.4)**2)<1.1:
                    model[i][j]=1
                    area=area+1

    return area
print(init_model("streamline"))
plt.figure()
...
for time in range (N):
    cnt=0
    model=Surfsand(watermode=1,left_probability=0.3,diff_dist=2)
    plt.figure(time*10+cnt,figsize=(8,8))
    sbn.heatmap(model,square=True)
    plt.savefig('F:/ameri/{}/{}.jpg'.format(kind,time*10+cnt))
    cnt=cnt+1
